...
<?php 
	...
	if (isset($_POST['submit_order'])) {
		...
		if ($hasil > 0) {
		    ...
		    $succes =<<<EOF
				<script>
				alert('Поздравляем, Вы обработали заказ.');
				window.history.go(-1);
				</script>
EOF;
			echo $succes;
...
<div id="content" class="pmd-content content-area dashboard">
	...
	<div class="container-fluid full-width-container">
		...
		<ol class="breadcrumb text-left">
		  ...
		  <li><a href="manage-order.php">Управление Заказами</a></li>
						...
						<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12 right">
							<div align="right">
								...
								<form id="validationForm" method="post">
									<input type="submit" name="cancel_order" class="btn pmd-ripple-effect btn-default" value="ОТМЕНА" onclick="cancelClicked(event)"/>

									<input type="submit" name="submit_order" class="btn pmd-ripple-effect btn-danger" value="ОБРАБОТАТЬ" onclick="processClicked(event)" />
								</form>
								...
								<?php } else if ($data['status'] == '2') { ?>
									<input type="submit" name="cancel_order" class="btn pmd-ripple-effect btn-default" value="ОТМЕНЁН" disabled/>
								<?php } ?>
							</div>
						</div>
					</div>
							...
							<tbody>
								...
								<tr>
									...
									<td>
										...
										<?php } else if ($data['status'] == '2') { ?>
										<span class="badge">ОТМЕНЁН</span>
										<?php } else { ?>
										<span class="badge badge-error">В ОЖИДАНИИ</span>
										<?php } ?>										
									</td>
								</tr>
...
<script>
	function processClicked(e) {
	    if(!confirm("Статус заказа не может быть изменен после обработки, если вы не отмените его."))e.preventDefault();
	}
</script>

<script>
	function cancelClicked(e) {
	    if(!confirm("Вы хотите отменить заказ <?php echo $data['name']; ?>"))e.preventDefault();
	}
</script>